...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const urlcheck = videoPattern.test(args[0]);//It is encoded by Murat Eren
    ...
    if (urlcheck) {
      try {
         ...
         const result = await youtube.searchVideos(args[0], 1)//It is encoded by Murat Eren
      ...
      try {
         const result = await youtube.searchVideos(targetsong, 1)//It is encoded by Murat Eren
    ...
    if(serverQueue) {
      ...
      return serverQueue.textChannel.send( new Discord.MessageEmbed()//It is encoded by Murat Eren
        .setColor("BLUE")
    ...
    if(!serverQueue) message.client.queue.set(message.guild.id, queueConstruct)
     ...
     if (!serverQueue) {
      try {
        ...
        await channel.leave();//It is encoded by Murat Eren
